...
# with open('./Dart_report_list.json', 'w', encoding = 'utf-8')  as f : # report list json 파일로 저장
#     json.dump(report_list, f, ensure_ascii=False, indent = '\t')

#이미 report_list.json 파일 있는 경우 사용
with open('./Dart_report_list.json', 'r', encoding = 'utf-8')  as f :
...
with open('./Dart_report_url.json', 'r', encoding = 'utf-8') as f :
    report_url = json.load(f)

report_data = Extractor.getReportData(report_url)

with open('./Dart_report_data.json', 'w', encoding = 'utf-8') as f :
