...
def edgar_summ(path: str):
    ...
    # output folder
    if not os.path.exists('Chatbot/Summarized_data/Edgar'):
        os.makedirs('Chatbot/Summarized_data/Edagr')
    # group화 한 목차 저장할 파일
    categorized_file = "Chatbot/Summarized_data/Edgar/edgar_contents_grouping.txt"
    ...
    for idx, file_name in enumerate(file_list):
        ...
        text_aggre=''
        output_path = f"Chatbot/Summarized_data/Edgar/test-{GPT_MODEL_NAME}-{file['company']}-{file['period_of_report']}.txt"
...
def dart_summ(path: str): # param : dart data report file
    ...
    # output folder
    if not os.path.exists('Chatbot/Summarized_data/Dart'):
        os.makedirs('Chatbot/Summarized_data/Dart')

    # group화 한 목차 저장할 파일
    categorized_file = "Chatbot/Summarized_data/Dart/dart_contents_grouping.txt"
    cff = open(categorized_file,"w+", encoding='UTF-8')
    ...
    for corp_name in file.keys():
        for report_num in tqdm(file[corp_name].keys(), desc = f"{corp_name}"):
            output_path = f"Chatbot/Summarized_data/Dart/test-{GPT_MODEL_NAME}-{corp_name}-{report_num}.txt"
